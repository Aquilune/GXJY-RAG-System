...
class ArbitrageData(models.Model):
    ...
    delivery_date = models.DateField()           # 交割日期
    inter_period_time = models.IntegerField()    # 跨期时间

    # 重写 save 方法，在保存数据到数据库之前进行套利相关的计算
    def save(self, near_code, far_code, *args, **kwargs):
        if near_code and far_code:

            provider = get_data_provider()
            self.near_month_price = provider.get_near_month_price(near_code)
            self.far_month_price = provider.get_near_month_price(far_code)
